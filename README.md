# Peasant Road - Because EPubs are nice.
...
State lives under `~/.config/peasant_road` (followed stories, scraped chapters,
...
peasant_road add https://www.royalroad.com/fiction/107917/sky-pride
peasant_road add -n "My Name" https://www.royalroad.com/fiction/12345/...

# Pull new chapters for every followed story
peasant_road pull [--throttle]

# Pull new chapters AND rebuild the EPUBs of stories that changed
peasant_road refresh [--throttle]

# Show what's new over a recent window (default 48h)
peasant_road report [--hours N]

# Build EPUB(s) to the current directory (interactive fzf selector if no IDs)
peasant_road build [FIC_ID...]
```
...
```sh
peasant_road serve [--port 4567] [--bind 127.0.0.1]
# or, from a checkout:
...
```sh
peasant_road install [--interval HOURS] [--port N] [--bind ADDR]
systemctl --user daemon-reload
systemctl --user enable --now peasant-road-web.service peasant-road-pull.timer
...
Override behaviour with `PEASANT_ROAD_SCHEDULER` (`off` / `internal` /
`external`) and the cadence with `PEASANT_ROAD_INTERVAL_HOURS` (default 6).

**macOS launchd.** `peasant_road schedule [--interval HOURS]` installs a launchd
